...
public class TransportShardBulkAction extends TransportWriteAction<BulkShardRequest, BulkShardRequest, BulkShardResponse> {
    ...
    public static WritePrimaryResult<BulkShardRequest, BulkShardResponse> performOnPrimary(
        ...
        for (int requestIndex = 0; requestIndex < request.items().length; requestIndex++) {
            if (isAborted(request.items()[requestIndex].getPrimaryResponse()) == false) {// TODO AL 监测主节点是否在之前被提前判断为失败
        ...
        for (int i = 0; i < request.items().length; i++) {
            ...
            switch (replicaItemExecutionMode(item, i)) {
                case NORMAL:
                    ...
                    location = syncOperationResultOrThrow(operationResult, location);// AL location ++
                    break;
                case NOOP:
                    break;
                case FAILURE:
    ...
    private static Engine.Result performOpOnReplica(DocWriteResponse primaryResponse, DocWriteRequest docWriteRequest,
        ...
        switch (docWriteRequest.opType()) {
            ...
            case INDEX:
                ...
                result = replica.applyIndexOperationOnReplica(primaryResponse.getSeqNo(),
                    primaryResponse.getVersion(),
                    indexRequest.versionType().versionTypeForReplicationAndRecovery(),
    ...
    static Engine.IndexResult executeIndexRequestOnPrimary(IndexRequest request, IndexShard primary,
                                                           MappingUpdatePerformer mappingUpdater) throws Exception {
        final SourceToParse sourceToParse =
            SourceToParse.source(request.index(),
                request.type(),
                request.id(),
                request.source(),
        ...
        return executeOnPrimaryWhileHandlingMappingUpdates(primary.shardId(), request.type(),
            () ->
                primary.applyIndexOperationOnPrimary(request.version(),
                    request.versionType(),
                    sourceToParse,
